...
{
  uint8_t i=0;
  static uint8_t CodeOne=0x7c;
  static uint8_t CodeZero=0x70;
  
  for (i=0;i<8;i++)
  {
    if((dat & 0x80)==0x80)
    {
      HAL_SPI_Transmit_DMA(&WS2812Bhspi, &CodeOne, 1);
    }
    else
    {
      HAL_SPI_Transmit_DMA(&WS2812Bhspi, &CodeZero, 1); 
    }
    dat=dat<<1;
  }
}
...
{
  uint8_t rst[24]={0};
  HAL_SPI_Transmit_DMA(&WS2812Bhspi, rst, 24);
}

void WS2812B_Init(void)//should >50us
{
  uint8_t ResCode[50]={0};
  HAL_SPI_Transmit_DMA(&WS2812Bhspi, ResCode, 50);
